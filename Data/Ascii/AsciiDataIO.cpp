...
EventList readData(const std::string &InputFilePath,
  ...
  for (size_t i = 0; i < PIDs.size(); ++i) {
    EvtList.Header.insert(std::make_pair(i, PIDs[i]));
  }
...
void writeData(const EventList &EvtList, const std::string &OutputFilePath,
  ...
  if (!AppendToFile) {
    ...
    std::vector<int> PIDs(EvtList.Header.size());
    for (auto x : EvtList.Header) {
      PIDs[x.first] = x.second;
    }
    for (auto x : PIDs)
      OutputStream << "\tPid: " << x << std::endl;
